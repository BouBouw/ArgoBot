...
execute: async (client, message, args) => {
    ...
    if(arr[0] === 'FR_fr') {
        ...
        if(!args[0]) return message.channel.send({ content: `⁉️ - ${message.author} veuillez faire la commande \`emojis create/remove/infos/edit>\`` })
        switch(args[0]) {
            case 'add': {
        ...
        switch(args[0]) {
            case 'add': {
                const emoji_args = args[1]
                if(!emoji_args) return message.channel.send({
                    content: `:x: - ${message.author} please provide an emoji to add to the server.`
                })
                ...
                for (const emojis of args) {
                    ...
                    if(getEmoji.id) {
                        const name = args[2]
                        if(!name) return message.channel.send({
                            content: `:x: - ${message.author} please provide a name for your emoji.`
                        })
                        const emojiExt = getEmoji.animated ? '.gif' : '.png';
                        const emojiUrl = `https://cdn.discordapp.com/emojis/${getEmoji.id + emojiExt}`;
                        message.guild.emojis.create(emojiUrl, name)
                            .then(async (emoji) => {
                                message.channel.send({ 
                                    content: `✅ - ${message.author} emoji <:${emoji.name}:${emoji.id}> correctly added as \`${emoji.name}\`.`
                                })
                            })
                            .catch((err) => {
                                message.channel.send({
                                    content: `:warning: - ${message.author} an error occurred while creating the emoji`
                                })
                            })
                    }
                }
            ...
            case 'remove': {
                const emojiID = args[1]
                if(!emojiID) return message.channel.send({
                    content: `:x: - ${message.author} please provide a valid server emoji.`
                })
                ...
                for (const emojis of args) {
                    ...
                    if(getEmoji.id) {
                        if(message.guild.emojis.cache.find(emoji => emoji.id === getEmoji.id)) {
                            const emoji = message.guild.emojis.cache.find(emoji => emoji.id === getEmoji.id)
                            message.channel.send({
                                content: `✅ - ${message.author} emoji ${emoji} successfully deleted from the server.`
                            }).then(async () => {
                                emoji.delete()
                            })
                            .catch((err) => {
                ...
                for (const emojis of args) {
                    ...
                    if(getEmoji.id) {
                        if(message.guild.emojis.cache.find(emoji => emoji.id === getEmoji.id)) {
                            ...
                            const newName = args[2]
                            if(!newName) return message.channel.send({
                                content: `:x: - ${message.author} please provide a new name for the emoji.`
                            })
                            ...
                            emoji.edit({
                                ...
                                message.chanel.send({
                                    content: `✅ - ${message.author} the name of the emoji ${emoji} just changed to \`:${newName}:\``
                                })
                            })
                            .catch((err) => {
                                message.channel.send({
                                    content: `:warning: - ${message.author} an error occurred while editing the emoji`
                                })
                            })
                        } else {
                            message.channel.send({
