...
execute: async (client, message, args) => {
    ...
    const array = db.get(`client_${client.user.id}_coinsSettings`)
                ...
                switch (collected.customId) {
                    ...
                    case 'normal': {
                        ...
                        message.channel.awaitMessages({ filter, max: 1, time: 30000, errors: ['time'] }).then(async (collected) => {
                            ...
                            if(db.get(`client_${client.user.id}_coinsSettings`) == null) {
                                db.set(`client_${client.user.id}_coinsSettings`, [`${collected.first().content}`])
                            } else {
                                array[0] = `${collected.first().content}`
                                await db.set(`client_${client.user.id}_coinsSettings`, array)
                            }
                    ...
                    case 'mute': {
                        ...
                        message.channel.awaitMessages({ filter, max: 1, time: 30000, errors: ['time'] }).then(async (collected) => {
                            ...
                            if(db.get(`client_${client.user.id}_coinsSettings`) == null) {
                                db.set(`client_${client.user.id}_coinsSettings`, [`10`, `${collected.first().content}`])
                            } else {
                                array[1] = `${collected.first().content}`
                                await db.set(`client_${client.user.id}_coinsSettings`, array)
                            }
                    ...
                    case 'stream': {
                        ...
                        message.channel.awaitMessages({ filter, max: 1, time: 30000, errors: ['time'] }).then(async (collected) => {
                            ...
                            if(db.get(`client_${client.user.id}_coinsSettings`) == null) {
                                db.set(`client_${client.user.id}_coinsSettings`, [`10`, `5`, `${collected.first().content}`])
                            } else {
                                array[2] = `${collected.first().content}`
                                await db.set(`client_${client.user.id}_coinsSettings`, array)
                            }
                    ...
                    case 'click': {
                        if(db.get(`client_${client.user.id}_coins`) == 'enable') {
                            ...
                            db.set(`client_${client.user.id}_coins`, 'enable')
                                ...
                                const state = await db.get(`client_${client.user.id}_coinsSettings`)
                                if(!state) {
                                    await db.set(`client_${client.user.id}_coinsSettings`, [`0`])
                                }
                        }
                        awaitButtons()
                        break;
