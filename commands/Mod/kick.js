        ...
        await user.kick({ reason: reason })
            ...
            async function sendLogs() {
                ...
                await logs.send({
                    embeds: [{
                        color: `#32a88b`,
                        description: `[\`EXPULSION\`] ${message.author} (\`${message.author.id}\`) à expulser ${user.tag} (\`${user.id}\`)`,
                        author: {
        ...
        await user.kick({ reason: reason })
            ...
            async function sendLogs() {
                ...
                await logs.send({
                    embeds: [{
                        color: `#32a88b`,
                        description: `[\`KICK\`] ${message.author} (\`${message.author.id}\`) has kicked ${user.tag} (\`${user.id}\`)`,
                        author: {
                            name: `${message.author.tag}`,
                            icon_url: `${message.author.avatarURL()}`,
                        },
