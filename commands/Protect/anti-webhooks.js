...
execute: async (client, message, args) => {
    ...
    if(arr[0] == 'FR_fr') {
        ...
        embed.setDescription(`🔘 **Activer/Désactiver l'anti-webhooks**`)
        embed.setTimestamp()
        ...
        message.channel.send({ embeds: [embed] }).then(async (msg) => {
            ...
            async function awaitButtons() {
                ...
                switch (collected.customId) {
                    case 'click': {
                        if(db.get(`client_${client.user.id}_security_webhooks`) == 'enable') {
                            db.set(`client_${client.user.id}_security_webhooks`, 'disable')
                                embed.setTitle(`Statut: \`❌\``)
                                await msg.edit({ embeds: [embed] })
                        } else {
                            db.set(`client_${client.user.id}_security_webhooks`, 'enable')
                                embed.setTitle(`Statut: \`✅\``)
                                await msg.edit({ embeds: [embed] })
                        }
                        awaitButtons()
        ...
        embed.setAuthor(message.author.tag, message.author.avatarURL({ dynamic: true }))
        embed.setTitle(`Statut: \`${statut}\``)
        embed.setDescription(`🔘 **Enable/Disable anti-webhooks**`)
        embed.setTimestamp()
        embed.setFooter(client.user.username)
        message.channel.send({ embeds: [embed] }).then(async (msg) => {
            const filter = (interaction) => interaction.user.id === message.author.id && interaction.isButton();
