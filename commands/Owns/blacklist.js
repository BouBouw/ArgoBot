                    ...
                    case 'add': {
                        ...
                        message.channel.awaitMessages({ filter, max: 1, time: 30000, errors: ['time'] }).then(async (collected) => {
                            ...
                            if(!array) {
                                ...
                                await message.channel.send({ content: `:white_check_mark: - ${message.author} l'utilisateur ${user} (\`${user.id}\`) à été ajouté à la liste noire.` }).then(async () => {
                        ...
                        message.channel.awaitMessages({ filter, max: 1, time: 30000, errors: ['time'] }).then(async (collected) => {
                            ...
                            if(!array) {
                                ...
                                if(!array.includes(user)) {
                                    ...
                                    const unbanUser = bannedUsers.get(user).user;
                                    await message.guild.members.unban(user).then(async () => {
                                        message.channel.send({ content: `:white_check_mark: - ${message.author} l'utilisateur ${unbanUser.tag} (\`${unbanUser.id}\`) à été retiré de la liste noire`})
                                    })
                                }
                            }
                        })
                ...
                switch (collected.customId) {
                    case 'add': {
                        ...
                        message.channel.awaitMessages({ filter, max: 1, time: 30000, errors: ['time'] }).then(async (collected) => {
                            ...
                            if(!array) {
                                db.set(`client_${client.user.id}_blacklist`, [`${user.id}`])
                                await message.channel.send({ content: `:white_check_mark: - ${message.author} the user ${user} (\`${user.id}\`) has been added to the blacklist.` }).then(async () => {
                                    await user.ban({ reason: `Blacklisted` })
                                })
                            } else {
                                if(!array.includes(user.id)) {
                            ...
                            if(!array) {
                                ...
                                if(!array.includes(user)) {
                                    ...
                                    db.set(`client_${client.user.id}_blacklist`, filtered)
                                        ...
                                        await message.guild.members.unban(user).then(async () => {
                                            message.channel.send({ content: `:white_check_mark: - ${message.author} the user ${unbanUser.tag} (\`${unbanUser.id}\`) has been removed to the blacklist.`})
                                        })
                                }
                            }
                        })
